...
const SYSTEM_PROMPT =
  ...
  "- Only if the user still genuinely needs something, suggest one better search direction.\n\n" +
  "Decision order:\n" +
  ...
  "- Keep each field short. 1 to 3 short sentences is enough. No long paragraphs.\n\n" +
  "Smartness rule:\n" +
  ...
  "- If no purchase is needed, leave alternative fields empty.\n\n" +
  "Field rules:\n" +
  "- verdict: one clear judgment sentence.\n" +
  "- why: 1 to 3 short sentences. Must include the sharpest product-specific insight.\n" +
  "- system_insight: 1 to 3 short sentences. Reveal the bigger pattern, but not in a generic way.\n" +
  "- action_step: one very concrete next step.\n" +
  "- alternative_name: a search direction, not a product name. Only if genuinely useful.\n" +
  "- alternative_reason: short reason why that direction is better.\n" +
      ...
      messages: [
        ...
        },
        {
          ...
          content:
            ...
            '"why":"1~3개의 짧은 문장. 가장 구체적인 insight 포함",\n' +
            '"system_insight":"1~3개의 짧은 문장. 더 큰 소비 패턴을 generic하지 않게 설명",\n' +
            '"action_step":"가장 좋은 다음 선택 한 가지",\n' +
            '"is_already_good":false,\n' +
            '"alternative_name":"정말 필요할 때만 한 가지 search direction, 아니면 빈 문자열",\n' +
            '"alternative_reason":"왜 그 방향이 더 나은지 짧게, 필요 없으면 빈 문자열",\n' +
            '"impact_of_switch":"현실적으로 무엇이 달라지는지 짧게, 필요 없으면 빈 문자열",\n' +
...
function normalizeResponse(parsed, product) {
  ...
  const keepAlternative = shouldKeepAlternative(outcomeType, parsed, alternativeFallback);
...
function normalizeOutcomeType(value, parsed, category) {
  ...
  if (category === "shampoo" || category === "detergent" || category === "cosmetics" || category === "airpods" || category === "iphone" || category === "smartphone") {
    return "reuse_refill";
  }
...
function shouldKeepAlternative(outcomeType, parsed, fallback) {
  ...
  if (outcomeType === "better_alternative") {
    return Boolean(name || keyword || fallback.name);
  }
...
function inferCategory(product, productName) {
  ...
  if (text.includes("휴대폰") || text.includes("스마트폰") || text.includes("smartphone") || text.includes("phone")) {
    return "smartphone";
  }
...
function getAlternativeFallback(category, outcomeType) {
  if (outcomeType !== "better_alternative" && outcomeType !== "reuse_refill") {
  ...
  switch (category) {
    ...
    case "cosmetics":
      return {
        name: "리필 화장품",
        reason: "정말 새로 사야 한다면, 적어도 같은 패키지를 반복해서 버이지 않는 방향이 낫습니다.",
